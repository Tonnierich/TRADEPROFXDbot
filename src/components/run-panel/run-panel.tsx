...
const RunPanel = observer(() => {
  ...
  // Effect to initialize drawer state on mobile
  React.useEffect(() => {
    if (!isDesktop) {
      // Close drawer by default on mobile, except for Analysis tab
  ...
  React.useEffect(() => {
    ...
    if (isInAnalysisTab) {
      ...
      // For Analysis tab, ensure the drawer is open
      if (!isDesktop && !is_drawer_open) {
        toggleDrawer(true)
      }
    } else {
  ...
  }, [active_tab, ANALYSIS, isDesktop, toggleDrawer, is_drawer_open])
  ...
  const handleToggleDrawer = () => {
    ...
    // Force the drawer to stay open for a moment to prevent flickering
    if (!wasOpen) {
  ...
  return (
    <>
      <div className={!isDesktop && is_drawer_open ? "run-panel__container--mobile" : "run-panel"}>
        <Drawer
          ...
          className={classNames("run-panel", {
            ...
            "run-panel__container--analysis": isAnalysisTab,
          })}
          contentClassName="run-panel__content"
          header={header}
          footer={isDesktop && footer}
          is_open={is_drawer_open || forceDrawerOpen}
          toggleDrawer={isDesktop ? toggleDrawer : handleToggleDrawer}
          width={366}
      ...
      {!isDesktop && isAnalysisTab && (
        <div id="mobile-drawer-toggle" className="mobile-drawer-toggle" onClick={handleToggleDrawer}>
          {is_drawer_open ? "▼" : "▲"}
        </div>
      )}
    </>
